  ...
  return (
    <Router>
      <div className="App">
        ...
        <Route path='/submissions/view/:index/' render={(props) => {
          return <ViewSubmissions name={props.location.state.name} url={props.location.state.url}/>
        }} />
      </div>
    </Router>
  );
...
function Home() {
  ...
  const images = [
    DogPhoto, JulyPhoto, DogPhoto, JulyPhoto, DogPhoto
  ];
      ...
      <header className="App-header">
        ...
        <UserList>
          {names.map((name, index) =>
            <UserCard name={name} key={name + 'KEY'} index={index + 1} img={images[index]} />)}
        </UserList>
        <Link to={'/submissions'}>
          <p>
            View Submissions
  ...
  return (
    <div>
      <Link className="User-card" to={`/user/${name}`}>
        ...
        <div className={width > height ? "User-image-landscape" : "User-image-portrait"}>
          <img src={img} onLoad={onImgLoad} alt="profile" /></div>
